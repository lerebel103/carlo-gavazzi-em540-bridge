...
# writes from its always-on supervisor loop (last successful upstream frame
# time, independent of MQTT), not a process-spawning functional check, so it
# does not perturb the 10Hz tick loop. The probe is observability only — Docker
# does not restart unhealthy containers. Automatic recovery is driven
# application-side: the bridge self-exits when no fresh upstream frame arrives
# within em540_master.health_max_stale_s, and the compose restart policy
# restarts it.
