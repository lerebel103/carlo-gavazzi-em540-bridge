...
def test_health_heartbeat_writes_last_frame_epoch_seconds(tmp_path):
    diagnostics = HADiagnostics(topic_prefix="test")
    health_file = tmp_path / "em540_health"
    diagnostics._health_file = str(health_file)

    # A frame observed at a known wall-clock time.
    frame = SimpleNamespace(timestamp=1_700_000_123.9)
    diagnostics.new_data(frame)

    with patch.dict("sys.modules", {"uptime": SimpleNamespace(uptime=lambda: 1)}):
        diagnostics.mqtt_data()

    # Written as integer epoch seconds (truncated), matching the shell probe.
    assert health_file.read_text() == "1700000123"


def test_health_heartbeat_reflects_only_last_observed_frame(tmp_path):
    diagnostics = HADiagnostics(topic_prefix="test")
    health_file = tmp_path / "em540_health"
    diagnostics._health_file = str(health_file)

    diagnostics.new_data(SimpleNamespace(timestamp=1_700_000_000.0))
    diagnostics.new_data(SimpleNamespace(timestamp=1_700_000_050.0))

    with patch.dict("sys.modules", {"uptime": SimpleNamespace(uptime=lambda: 1)}):
        diagnostics.mqtt_data()

    # The heartbeat tracks the most recent frame, not a wall-clock "now": if
    # frames stop arriving, the value stays frozen and the probe goes stale.
    assert health_file.read_text() == "1700000050"


def test_health_heartbeat_is_zero_before_any_frame(tmp_path):
    diagnostics = HADiagnostics(topic_prefix="test")
    health_file = tmp_path / "em540_health"
    diagnostics._health_file = str(health_file)

    # No new_data() call yet — mimics cold start before the first frame lands.
    with patch.dict("sys.modules", {"uptime": SimpleNamespace(uptime=lambda: 1)}):
        diagnostics.mqtt_data()

    # Zero reads as maximally stale, which the healthcheck treats as unhealthy
    # (absorbed by start_period during boot).
    assert health_file.read_text() == "0"


def test_health_heartbeat_write_failure_does_not_break_diagnostics():
    diagnostics = HADiagnostics(topic_prefix="test")
    # Point at an unwritable path (missing parent directory) to force a failure.
    diagnostics._health_file = "/nonexistent-dir-xyz/em540_health"
    diagnostics.new_data(SimpleNamespace(timestamp=1_700_000_000.0))

    with patch.dict("sys.modules", {"uptime": SimpleNamespace(uptime=lambda: 1)}):
        # Must still return a valid diagnostics payload despite the write error.
        _, payload = diagnostics.mqtt_data()
