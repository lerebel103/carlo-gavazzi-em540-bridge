...
class SystemData:
    ...
    def parse(self, registers):
        ...
        self.line_neutral_voltage = ModbusTcpClient.convert_from_registers(registers[0x024: 0x024 + 2],
                                                                           ModbusTcpClient.DATATYPE.INT32,
                                                                           "little") / 10
        self.line_line_voltage = ModbusTcpClient.convert_from_registers(registers[0x026: 0x026 + 2],
                                                                        ModbusTcpClient.DATATYPE.INT32, "little") / 10
        self.power = ModbusTcpClient.convert_from_registers(registers[0x028: 0x028 + 2], ModbusTcpClient.DATATYPE.INT32,
                                                            "little") / 10
        self.apparent_power = ModbusTcpClient.convert_from_registers(registers[0x02A: 0x02A + 2],
                                                                     ModbusTcpClient.DATATYPE.INT32, "little") / 10
        self.reactive_power = ModbusTcpClient.convert_from_registers(registers[0x02C: 0x02C + 2],
                                                                     ModbusTcpClient.DATATYPE.INT32, "little") / 10
        self.power_factor = ModbusTcpClient.convert_from_registers(registers[0x031: 0x031 + 1],
...
class PhaseData:
    ...
    def parse(self, phase_idx, registers):  # Each block of 7 registers corresponds to a phase:
        ...
        self.line_neutral_voltage = ModbusTcpClient.convert_from_registers(registers[i: i + 2],
                                                                           ModbusTcpClient.DATATYPE.INT32,
                                                                           "little") / 10.0

        i = phase_idx * 2 + 0x0006
        self.line_line_voltage = ModbusTcpClient.convert_from_registers(registers[i: i + 2],
                                                                        ModbusTcpClient.DATATYPE.INT32, "little") / 10.0

        i = phase_idx * 2 + 0x000C
        self.current = ModbusTcpClient.convert_from_registers(registers[i: i + 2], ModbusTcpClient.DATATYPE.INT32,
                                                              "little") / 1000.0

        i = phase_idx * 2 + 0x0012
        self.power = ModbusTcpClient.convert_from_registers(registers[i: i + 2], ModbusTcpClient.DATATYPE.INT32,
                                                            "little") / 10.0

        i = phase_idx * 2 + 0x0018
        self.apparent_power = ModbusTcpClient.convert_from_registers(registers[i: i + 2],
                                                                     ModbusTcpClient.DATATYPE.INT32, "little") / 10.0

        i = phase_idx * 2 + 0x001E
        self.reactive_power = ModbusTcpClient.convert_from_registers(registers[i: i + 2],
                                                                     ModbusTcpClient.DATATYPE.INT32, "little") / 10.0

        i = phase_idx + 0x002E
        self.power_factor = ModbusTcpClient.convert_from_registers(registers[i: i + 1], ModbusTcpClient.DATATYPE.INT16,
                                                                   "little") / 1000.0
